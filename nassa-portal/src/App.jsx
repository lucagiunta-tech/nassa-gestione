...
async function createDropboxClientFolders(clientName) {
  ...
  if (!token) return;
  const base  = `${DROPBOX_FOLDER}/${toFolderName(clientName)}`;
  const paths = [`${base}/Images`, `${base}/Videos`, `${base}/Documents`];
  try {
    await fetch("https://api.dropboxapi.com/2/files/create_folder_batch", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ paths, autorename: false }),
    });
  } catch { /* non-blocking */ }
}
...
async function uploadToDropbox(file, clientName, subfolder = "Images") {
  const token = await getDropboxToken();
  if (!token) throw new Error("Dropbox non configurato — controlla DROPBOX_APP_KEY e DROPBOX_REFRESH_TOKEN");
  ...
  // ── Step 2: create (or fetch) a public shared link ────────
  let publicUrl = null;
  ...
  try {
    const createRes  = await fetch("https://api.dropboxapi.com/2/sharing/create_shared_link_with_settings", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ path: upData.path_lower }),
    });
    const createData = await createRes.json();
    publicUrl = createData.url || null;
  ...
  if (!publicUrl) {
    const listRes  = await fetch("https://api.dropboxapi.com/2/sharing/list_shared_links", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ path: upData.path_lower, direct_only: true }),
    });
    const listData = await listRes.json();
    publicUrl = listData.links?.[0]?.url || null;
  }

  if (!publicUrl) throw new Error("Impossibile creare il link Dropbox");
...
function openMetaOAuth(onToken) {
  ...
  const url =
    ...
    `&redirect_uri=${redirect}&scope=${META_SCOPES}&response_type=token`;
...
function MetaConnectionSection({ slug }) {
  ...
  function startOAuth() {
    setConnecting(true);
    openMetaOAuth(async (shortToken) => {
      try {
        const ll   = await metaExchangeLongLived(shortToken);
        const pages = await metaGetPages(ll.access_token);
        setPagesData({ userToken: ll.access_token, pages: pages.data || [] });
      } catch (e) {
...
export default function App() {
  ...
  useEffect(() => {
    // ── Handle Meta OAuth popup callback ─────────────────────
    // When Facebook redirects back with #access_token=..., this page
    // may be loaded inside the popup. Detect it and postMessage to opener.
    ...
    if (hash.includes("access_token=") && window.opener) {
      ...
      if (token) {
        ...
        window.close();
        return; // Don't init the app — this tab will close
      }
    }
    // ── Normal app init ───────────────────────────────────────
